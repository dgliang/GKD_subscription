...
if __name__ == "__main__":
    # 设置命令行参数解析
    parser = argparse.ArgumentParser(description='处理匹配元素数据')
    parser.add_argument('task_id', type=str, help='任务 task_id')
    
    args = parser.parse_args()
    
    # 构建目标目录路径
    target_directory = os.path.join("..", "poker", "collectData", args.task_id)
    ...
    if not os.path.isdir(target_directory):
        print(f"错误：路径不是目录: {target_directory}")
        exit(1)
    
    print(f"处理任务ID: {args.task_id}")
